...
{
    // Convert Specie:// to Specie:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Specie://"))
    {
        uri.replace(0, 12, "Specie:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Specie.lnk";
}
...
{
    return GetAutostartDir() / "Specie.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Specie-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Specie-qt [" + tr("command-line options") + "]                     " + "\n";
